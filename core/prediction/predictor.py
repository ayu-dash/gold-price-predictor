...
def train_model(X: pd.DataFrame, y: pd.Series, quantile: Optional[float] = None):
    """Train a high-precision Stacking Ensemble (Best Known Config)."""
    # 10% test size for focused precision
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, shuffle=False)

    if quantile is not None:
        model = HistGradientBoostingRegressor(
            learning_rate=0.01, max_depth=10, max_iter=1000, loss='quantile',
            quantile=quantile, l2_regularization=1.0,
            early_stopping=True, validation_fraction=0.1, random_state=42
        )
    else:
        # Median (Main Model) uses 2066-Success Stacking
        estimators = [
            ('hgb', HistGradientBoostingRegressor(
                loss='absolute_error', learning_rate=0.01, max_depth=10, 
                max_iter=1000, l2_regularization=1.5, early_stopping=True, random_state=42
            )),
            ('et', ExtraTreesRegressor(n_estimators=300, max_depth=12, random_state=42)),
            ('gb', GradientBoostingRegressor(loss='absolute_error', n_estimators=300, learning_rate=0.03, random_state=42))
        ]
        
        # Meta-learner: Ridge (More stable than Lasso for these base models)
        final_est = make_pipeline(StandardScaler(), Ridge(alpha=1.0))
